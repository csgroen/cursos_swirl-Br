  ...
  if(is.null(correctName)){
    results <- expectThat(length(delta) >= 1,
                          ...
                          label=paste(deparse(e$expr), 
                                      "não criou a variável."))  
  } else {
    results <- expectThat(correctName %in% names(delta), 
                          testthat::is_true(), 
                          label=paste(deparse(e$expr),
                                      "não criou a variável nomeada",
                                      correctName))
  }
  if(results$passed){
    e$newVar <- e$val
...
omnitest <- function(correctExpr=NULL, correctVal=NULL, strict=FALSE){
  ...
  if(valGood && exprGood){
    ...
    swirl_out("Esta não é a expressão que eu esperava, mas funciona.")
    swirl_out("Eu executei a expressão correta no caso do resultado ser necessário em questões futuras.")
...
notify <- function() {
  ...
  while(!good) {
    # Get info
    name <- readline_clean("Qual é o seu nome? ")
    address <- readline_clean("ual é o email da pessoa que você gostaria de informar? ")
